...
int main()
{
  try {
    ...
    {
      const EthernetAddress local_eth = random_private_ethernet_address();
      NetworkInterfaceTestHarness test { "Make sure there are no pending datagrams", local_eth, Address( "4.3.2.1", 0 ) };
      ...
      // Reply should result in the queued datagram getting sent => TYPE_IPv4 reply.
      test.execute( ReceiveFrame {
        make_frame(
          target_eth,
          local_eth,
      ...
      // There should be no more pending datagrams in queue => TYPE_ARP reply.
      test.execute( ReceiveFrame {
        make_frame(
          target_eth,
          local_eth,
